...
func checkNoDuplicateNames(t *testing.T, code string) {
	t.Helper()
	re := regexp.MustCompile(`(?:struct|const|fn|var|let|alias)\s+(\w+)`)
	...
	for name, count := range names {
		if count > 1 {
			t.Errorf("duplicate declaration name %q appears %d times in output:\n%s", name, count, code)
		}
	}
}

func TestNoDuplicateNamesBasic(t *testing.T) {
	source := `
struct S1 { x: f32 }
struct S2 { y: f32 }
const c1: f32 = 1.0;
...
`
	...
	checkNoDuplicateNames(t, result.Code)
}

// Test with many symbols that could exhaust short names and cause collisions
func TestNoDuplicateNamesManySymbols(t *testing.T) {
	source := `
...
const state_b = vec2f();
const offsets_a: array<vec3f, 7> = array(
  vec3f(-0.25, 0.0, -3.141592653589793 * 0.25),
  vec3f(0.0, 0.8, -0.18),
  vec3f(-0.8, 0.3, -0.18),
  vec3f(0.6, -0.6, 0.33),
  vec3f(0.5, 0.2, 0.1),
...
const offsets_b: array<vec3f, 7> = array(
  vec3f(0.83, -0.30, -0.65),
  vec3f(0.06, -0.20, -0.36),
  vec3f(-0.23, 0.38, -0.71),
  vec3f(-0.83, -0.18, -0.26),
  vec3f(-0.59, -0.47, 0.05),
  vec3f(-0.14, -0.94, 0.95),
  vec3f(0.24, -0.89, -0.58)
);
const offsets_c: array<vec3f, 7> = array(
  vec3f(0.59, -0.07, 0.35),
  vec3f(0.25, 0.08, 0.54),
  vec3f(-0.96, 0.59, -0.94),
  vec3f(0.27, -0.83, -0.30),
  vec3f(0.99, 0.68, -0.82),
  vec3f(0.06, 0.17, 0.74),
  vec3f(0.87, 0.73, 0.91)
);
const offsets_d: array<vec3f, 7> = array(
  vec3f(0.73, -0.18, -0.88),
  vec3f(-0.64, 0.24, 0.69),
  vec3f(0.49, 0.68, -0.27),
  vec3f(0.02, 0.76, 0.78),
  vec3f(-0.002, 0.47, 0.58),
  vec3f(0.46, 0.93, 0.43),
  vec3f(-0.58, -0.32, -0.41)
);
const offsets_e: array<vec3f, 7> = array(
  vec3f(0.46, -0.91, -0.56),
  vec3f(0.35, 0.35, 0.63),
  vec3f(-0.17, -0.93, 0.52),
  vec3f(-0.95, 0.29, 0.91),
  vec3f(-0.48, -0.94, -0.45),
  vec3f(-0.64, -0.01, 0.49),
  vec3f(-0.24, 0.74, -0.54)
);
const offsets_f: array<vec3f, 7> = array(
  vec3f(-0.55, 0.12, -0.34),
  vec3f(0.62, 0.17, -0.35),
  vec3f(0.31, -0.95, 0.66),
  vec3f(0.61, -0.78, 0.46),
  vec3f(-0.24, 0.54, 0.36),
  vec3f(0.05, 0.93, 0.12),
  vec3f(-0.98, 0.96, 0.95)
);
const offsets_g: array<vec3f, 7> = array(
  vec3f(0.98, -0.55, 0.68),
  vec3f(-0.03, -0.63, 0.52),
  vec3f(-0.83, 0.05, -0.67),
  vec3f(-0.19, 0.19, -0.32),
  vec3f(0.81, -0.80, 0.66),
  vec3f(0.58, -0.76, -0.30),
  vec3f(-0.38, -0.50, -0.45)
);
